...
import { registerPaidAds } from './adPopupStore';
import type { Ad as PopupAd } from './adsData';
...
const CACHE_KEY = 'ch_approved_ads_v1';
...
// RightAdBanner) — בלי להמתין ל-onMount של רכיב כלשהו.
if (browser) loadApprovedAds();

// ===== הזנת הפופ-אפ בנייד =====
// למנגנון הקיים של האתר (adPopupStore) יש כבר דילול "מפרסמים משולמים
// מול פרסומות רשת" — עד היום לא היה לו מקור מודעות משולמות. כאן
// המודעות המאושרות מהבילדר נרשמות אצלו, והדילול נכנס לפעולה מעצמו.
// הטקסטים עבריים בכל השפות בכוונה: תוכן המפרסם אינו מתורגם.
if (browser) {
    approvedAds.subscribe((list) => {
        const heb = (s: string) => ({ he: s, en: s, ru: s });
        const paid: PopupAd[] = list
