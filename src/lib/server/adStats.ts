...
const STATS_CATEGORY = '__ch_ad_stats';
...
async function loadStats(): Promise<{ id: string | null; store: RawStore }> {
    try {
        ...
        if (!(e instanceof StrapiContentTypeError)) {
            console.error('[chachmim] adStats load failed:', e);
        }
    ...
    try {
        ...
        if (id) {
            ...
            const res = await strapiPost<{ data: StrapiItem }>('/api/items', {
                data: {
                    label: 'chachmim-ad-stats',
                    category: STATS_CATEGORY,
                    description: '[SYSTEM] מדדי פרסומות — חכמי העדה',
                    icon: '📊',
        ...
        console.error('[chachmim] adStats flush failed:', e);
    } finally {
        flushing = false;
    }
}
