<script lang="ts">
    ...
    // ============================================================
    // הטור הימני — מהיום מונע-משבצות: מודעות בתשלום מהבילדר
    // (/about/advertise/builder, דרך Strapi) תופסות מקומות ממוספרים,
    // והמקומות הפנויים ממשיכים להציג את "מקום פרסום זה" הרב-לשוני
    // הקיים. פורט מפינת האבדות; המשבצות הפנויות נשארו בשפת העיצוב
    // (וה-i18n) של האתר הזה.
    // ============================================================
    ...
    const VIEW_MS = 7000;    // כמה זמן כל קבוצה נשארת על המסך

    let currentGroup = $state(0);

    let slots = $derived($adSlots);

    // לוח 16 המקומות בסדר מספרי, בקבוצות עוקבות של 4 (1-4, 5-8, ...):
    // מודעה שנקבעה למקום 5 מופיעה עם קבוצת 5-8. כל הכרטיסים מתחלפים
    // בסבב — מודעות ומשבצות פנויות יחד. הסבב רץ כל עוד הדף פתוח: גם
    // המודעות בתשלום מתחלפות בו, ועצירה הייתה מקבעת קבוצה אחת ומסתירה
    // לצמיתות את המודעות שבאחרות.
...
<aside
    aria-label={tFn('right_ad_aside_label')}
    ...
    <div class="ads-stage">
        ...
        {#each grp as item, index (item.kind === 'real' ? item.ad.id : `${gi}-${index}`)}
                ...
                >
                    <div class="flex-1 relative overflow-hidden bg-black/30">
                        ...
                        <!-- אותן שכבות בדיוק של הדמו החי בבילדר: רצועה אלכסונית
                             בגובה שהמפרסם כיוון, כותרת בצבע ובמיקום שבחר, סלוגן
                             מעל הרצועה, ולוגו בצורה ובמקום שקבע (כולל גרירה
                             חופשית). -->
                        ...
                        <!-- שכבת הריחוף: "טקסט הריחוף" מהבילדר — התוכן הנוסף שנועד למכור -->
                        <div
            ...
            {:else if item.kind === 'pending'}
                ...
                <div
                    class="h-[490px] rounded-2xl border border-amber-900/10 bg-amber-900/5 animate-pulse"
                    aria-hidden="true"
                ></div>
            {:else}
                {@const ad = item.slot}
                <!-- משבצת פנויה — הכרטיס הרב-לשוני הקיים של האתר; כולו קישור לדף הפרסום -->
                <a
                    href={ad.href}
                    aria-label="{tFn('right_ad_slot_headline')} — {tFn('right_ad_slot_subtext')}"
...
<style>
    ...
    /* ============================================================
       ...
       (about/advertise/builder: .pro-diag / .pro-title-top / .pro-sub /
       .ad-logo). המשתנים --diag-top-* מגיעים מהעיצוב ששמור עם המודעה
       עצמה, ולכן לכל מודעה גובה רצועה משלה - בדיוק כפי שהמפרסם כיוון.
       הכרטיס עצמו נושא את הצבעים של המפרסם ולכן נשאר זהה בכל ערכת אתר.
       ============================================================ */
    .promo-diag {
        position: absolute;
        inset: 0;
