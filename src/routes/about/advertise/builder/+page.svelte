<script>
    ...
    // בונה הפרסומות - פורט מפינת האבדות (במקור מ"קהילה בשכונה") והותאם לחכמי העדה:
    // עברית בלבד, CSS רגיל (בלי Tailwind), גרדיאנטים כמחרוזות CSS.

    import { getCurrentUser, isChachmeiAdmin } from "$lib/strapi";

    // אין סשן-שרת באתר הזה: המשתמש המחובר נטען בצד הלקוח (ה-JWT
    // מ-localStorage). משמש לתצוגה בלבד (תג אדמין, זיהוי בטופס העזרה)
    // — האכיפה האמיתית בצד השרת (adsAdmin.ts).
    /** @type {{ email: string | null, name: string | null } | null} */
    let layoutUser = $state(null);

    // ===== שמירה מקומית =====
    const LS_KEY = "ch_ad_builder_draft_v1";
    const PAID_KEY = "ch_ad_paid";
    const PAID_AT_KEY = "ch_ad_paid_at";
    ...
    const PLAN_DAYS_KEY = "ch_ad_plan_days";
    ...
    onMount(() => {
        ...
        // המשתמש המחובר — לתצוגה בלבד; רץ אחרי טעינת הטיוטה (אסינכרוני)
        // ולכן לא דורס אימייל שכבר הוקלד/נשמר בטיוטה.
        void getCurrentUser().then((u) => {
            if (!u) return;
            layoutUser = { email: u.email ?? null, name: u.nickname || u.username || null };
            isAdmin = isChachmeiAdmin(u);
    ...
    function openHelp() {
        helpProblem = "";
        helpContact = layoutUser?.email ?? layoutUser?.name ?? "";
        helpError = "";
        helpOpen = true;
        queueMicrotask(() => {
    ...
    function submitHelp() {
        ...
        const isLoggedIn = Boolean(layoutUser?.email || layoutUser?.name);
        if (!isLoggedIn && !contact) {
            helpError = "נא להשאיר דרך ליצירת קשר";
            return;
        }
        helpError = "";
        const identity = contact || layoutUser?.email || layoutUser?.name || "אנונימי";
        const draftSummary = buildDraftSummary();
        const waText =
            `שלום! אני צריך/ה עזרה בעיצוב הפרסומת באתר חכמי העדה 🆘\n\n` +
            `🧑 ${identity}\n\n` +
            `הבעיה:\n"${problem}"` +
...
<svelte:head>
    <title>בניית הפרסומת שלי | חכמי העדה</title>
</svelte:head>
...
{#if helpOpen}
    ...
    >
        <div class="help-modal" dir="rtl">
            ...
            {#if !layoutUser?.email && !layoutUser?.name}
                <label class="help-field">
            ...
            {:else}
                <p class="help-identity">
                    <span aria-hidden="true">🧑</span>
                    מזוהה כ-<strong>{layoutUser?.name || layoutUser?.email}</strong>
                </p>
            {/if}
...
<style>
    ...
    .b-countdown {
        ...
        background: linear-gradient(135deg, rgba(120, 53, 15, 0.25), rgba(124, 45, 18, 0.15)), #0f1626;
    }
    .b-expired {
        border: 1px solid rgba(239, 68, 68, 0.4);
        background: linear-gradient(rgba(239, 68, 68, 0.1), rgba(239, 68, 68, 0.1)), #0f1626;
    }
    .b-autosave {
        margin-top: 0.75rem;
        border: 1px solid rgba(34, 197, 94, 0.4);
        background: linear-gradient(rgba(34, 197, 94, 0.08), rgba(34, 197, 94, 0.08)), #0f1626;
    }
    /* מצב עריכה של מודעה קיימת — כחול, נבדל מהתראות התוקף הכתומות */
    .b-edit-mode {
        border: 2px solid rgba(96, 165, 250, 0.5);
        background: linear-gradient(rgba(59, 130, 246, 0.12), rgba(59, 130, 246, 0.12)), #0f1626;
    }
    ...
    /* ============== כרטיס שלב ============== */
    .step-card {
        background: linear-gradient(180deg, rgba(255, 255, 255, 0.03), rgba(255, 255, 255, 0.01)), #0f1626;
        border: 1px solid #2c3a55;
