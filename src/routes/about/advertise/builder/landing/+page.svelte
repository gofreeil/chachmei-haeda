<script>
    ...
    import { getCurrentUser, getJwt } from "$lib/strapi";

    // אין סשן-שרת: ההתחברות נבדקת בצד הלקוח לתצוגה (שער ההתחברות),
    // והשליחה עצמה נושאת Authorization Bearer שהשרת מאמת מול Strapi.
    /** @type {{ email: string | null, name: string | null } | null} */
    let layoutUser = $state(null);

    /** כותרת ההזדהות לשרת — אותו JWT שכל קריאות ה-needAuth באתר שולחות */
    ...
    onMount(() => {
        ...
        // ההתחברות נבדקת מול Strapi — רץ אחרי טעינת הטיוטה (אסינכרוני)
        // ולכן לא דורס אימייל שכבר נשמר בטיוטה.
        void getCurrentUser().then((u) => {
            loggedIn = Boolean(u);
            if (u) layoutUser = { email: u.email ?? null, name: u.nickname || u.username || null };
    ...
    function goLoginToSubmit() {
        ...
        void goto(`/login?returnTo=${encodeURIComponent("/about/advertise/builder/landing")}`);
    }
</script>
            ...
            <section class="step-card">
                ...
                {#if editId}
                    ...
                    <div class="pay-box">
                        ...
                        <a href="/about/advertise/manage/{editId}" class="pay-wa" style="background:#2c3a55">
                ...
                <div class="pay-box">
                    ...
                    {:else}
                        ...
                        <a
                            href={"https://wa.me/972587448061?text=" + encodeURIComponent(`שלום, אני מעלה פרסומת באתר חכמי העדה ורוצה לתאם תשלום עבור ${payPlan.title} (${payPlan.price} ₪)`)}
                            target="_blank"
...
<style>
    ...
    .l-badge {
        ...
        border-radius: 999px;
        background: linear-gradient(to right, rgba(245, 158, 11, 0.2), rgba(249, 115, 22, 0.2)), #0f1626;
